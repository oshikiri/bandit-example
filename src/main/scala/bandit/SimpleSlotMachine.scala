...
// TODO: 途中で腕の本数が変わるケース?
abstract class SimpleSlotMachine(
  val stateWithExpectedRewards: Map[SimpleBanditArm, Reward],
  val seed: Int
) {
  ...
  def pullArm(armId: ArmId): Reward = {
    stateWithExpectedRewards
      .filter { case (arm: SimpleBanditArm, expectedReward: Reward) =>
        arm.armId == armId
      }
      .map { case (arm: SimpleBanditArm, expectedReward: Reward) =>
