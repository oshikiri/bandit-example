...
class LognormalTS extends SimpleBanditAlgorithm {
    ...
    arms.map {
      case arm if arm.armId == choosedArmId =>
        ...
        val (muNew, sigmaNew) = updateParameters(
          arm.distribution.mu,
          arm.distribution.sigma,
          arm.sampleVarianceOfLogRewards,
          reward
        )
  ...
  private def updateParameters(muOld: Double,
                               sigmaOld: Double,
                               sampleVarianceOfLogRewards: Double,
                               reward: Reward): (Double, Double) = {
    val sigmaSquaredOld = pow(sigmaOld, 2)
    val sigmaSquaredNew =
      sigmaSquaredOld * sampleVarianceOfLogRewards / (sigmaSquaredOld + sampleVarianceOfLogRewards)
    val muNew = sigmaSquaredNew * (muOld / sigmaSquaredOld + log(reward) / sampleVarianceOfLogRewards + 0.5)

    val sanitizedMuNew = sanitizeMu(muNew, muOld)
    val sanitizedSigmaSquaredNew =
      sanitizeSigmaSquared(sigmaSquaredNew, sigmaSquaredOld)
  ...
  private def sanitizeSigmaSquared(sigmaSquared: Double, default: Double) =
    if (sigmaSquared.isNaN || sigmaSquared.isInfinity || sigmaSquared <= 0)
      default
    else sigmaSquared
}
...
object LognormalTS extends SimpleBanditAlgorithm {
  ...
  case class LognormalBanditState(arms: Seq[LognormalBanditArm], nTrials: Long)
      extends SimpleBanditState

  case class LognormalBanditArm(armId: ArmId,
                                nChoosed: Long,
                                sumOfRewards: Double,
